# v5 - abstract <sep> title <sep> non_citation_context [order preservation random selection]
...
def process_data(file, path):
    ...
    SRC_MAX_LEN = 800
    non_citation_idxs_list, srcs, src_vecs, trgs, trg_vecs = [], [], [], [], []

    for i in range(len(data)):
        non_citation_idxs = [idx for idx, item in enumerate(data[i]['tag']) if item == 'NC']
        non_citation_idxs_list.append(non_citation_idxs)
        ...
        non_cite_sentences     = [sentences[idx] for idx in non_citation_idxs]
        non_cite_sentences_vec = [sentences_vec[idx] for idx in non_citation_idxs]
        ...
        src_len            = sum([len(element) + 1 for element in non_cite_sentences]) + len(src[:indices[1]])

        if src_len > SRC_MAX_LEN:
            
            if non_cite_sentences != []:
                # recreate src (list of words) of max length SRC_MAX_LEN
                # 1) keep title & abstract as it is
                # 2) order preserved random selection for non-citation sentences

                scarce_factor = SRC_MAX_LEN - len(src)
                sublist_idxs  = []
                idxs          = [*range(len(non_cite_sentences))]
                counter       = 0
                while(1):
                    counter += 1
                    if counter >= 8:    break
                    ...
                    text         = [non_cite_sentences[i] for i in sublist_idxs]
                ...
                for idx in sublist_idxs:
                    ...
                    src.extend(non_cite_sentences[idx])
                    src_vec.extend(non_cite_sentences_vec[idx])

        else:   # simply append non-citations to title and abstract
            src          = src[:indices[1]]
            src_vec      = src_vec[:indices[1]]
            sep_token_id = src_vec[indices[0]]
            for idx in range(len(non_cite_sentences)):
                src.append('<sep>')
                src_vec.append(sep_token_id)
                src.extend(non_cite_sentences[idx])
                src_vec.extend(non_cite_sentences_vec[idx])
...
dump_folder = '../stage2_processed_data/v5/'
